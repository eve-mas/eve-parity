...
/* Output for EVE */
void print_buchi_eve(BState *s) /* dumps the Buchi automaton */
{
  BTrans *t;
  if(s == bstates) return;

  print_buchi_eve(s->nxt); /* begins with the last state */

  fprintf(tl_out, "state ");
  if(s->id == -1)
    if(s->final == accept)
       fprintf(tl_out, "init acc");
    else
       fprintf(tl_out, "init noacc");
  else {
    if(s->final == accept)
...
void print_eve_buchi() {
  ...
  if(bstates->nxt->nxt == bstates && bstates->nxt->id == 0) { /* true */
    fprintf(tl_out, "init accepting\n");
    fprintf(tl_out, "\t:: [true] -> goto init\n");
    return;
  }
